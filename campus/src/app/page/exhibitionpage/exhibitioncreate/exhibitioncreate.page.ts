...
export class ExhibitioncreatePage {
  ...
  members: { name: string; image: File | null; preview: string; generation: string; }[] = []; // 멤버 배열 수정
  outputImagesList: string[] = []; // 출력 이미지 URL 배열
  outputImages: FileList | null = null; // 출력 이미지 파일 목록
  ...
  // Member 추가
  addMember() {
    if (this.memberName && this.memberImage && this.memberGeneration) {
      const reader = new FileReader();
      reader.onload = (e) => {
        if (e.target && e.target.result) {
          this.members.push({
            name: this.memberName,
            image: this.memberImage, // File 객체를 저장
            preview: e.target.result as string, // 데이터 URL로 미리보기 저장
            generation: this.memberGeneration // 멤버 세대
          });
          // 초기화
          this.memberName = '';
          this.memberImage = null; // 초기화
          this.memberGeneration = '';
        }
      };
      reader.readAsDataURL(this.memberImage); // File 객체를 데이터 URL로 읽기
    }
  }
    ...
    this.exhibitionService.saveExhibitionData(exhibitionData).subscribe({
      next: (exhibitionResponse) => {
        ...
        this.members.forEach((member, index) => {
          ...
          membersData.append(`members[${index}][generation]`, member.generation);
          if (member.image) {
            membersData.append(`members[${index}][image]`, member.image);
          }
        });
